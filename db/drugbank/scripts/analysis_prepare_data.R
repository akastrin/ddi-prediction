...
sim_atc <- as.matrix(fread("./data/atc_similarity.txt"))
rownames(sim_atc) <- colnames(sim_atc)

# Read similarity matrix CHEMICAL
sim_chem <- as.matrix(fread("./data/chemical_similarity.txt"))
rownames(sim_chem) <- colnames(sim_chem)

# Read similarity matrix MeSH
sim_mesh <- as.matrix(fread("./data/mesh_similarity.txt"))
rownames(sim_mesh) <- colnames(sim_mesh)

# Read similarity matrix ADE
sim_ade <- as.matrix(fread("./data/sider_similarity.txt"))
